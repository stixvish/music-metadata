...
def _overrides_for(
  map_path: Path, store: Store | None = None
  ...
  **Only values the operator actually typed are returned.** The map holds
  generated values too, and returning those would make the pipeline read its
  own output back as an assertion — a generated artwork URL would be re-fetched
  as though the operator had chosen it, and its provenance would read `manual`.
  §9b's own test decides it: a value that differs from what was generated last
  ...
  Args:
    ...
    store: the sidecar, holding what was generated last run. Without it every
      value is treated as generated, so nothing overrides anything.
  ...
  for md5, values in on_disk.items():
    generated = store.get_generated(md5)
    edited = {
      name: value
      for name, value in values.items()
      if value and value != generated.get(name, "")
    }
    if edited:
      manual[md5] = edited
  return manual
...
def cmd_resolve(args: argparse.Namespace) -> int:
  ...
  with Store.open(args.sidecar) as store:
    ...
    overrides = _overrides_for(args.map, store)
